---
title: Contributing to gstack
description: How to contribute to gstack - development setup, testing, and submitting PRs
---
...
- **Generator tests** (`test/gen-skill-docs.test.ts`) — Tests the template system: verifies placeholders resolve correctly, output includes value hints for flags (e.g. `-d <N>` not just `-d`), enriched descriptions for key commands.
...
Now open Claude Code in the other project — it picks up skills from `~/.claude/skills/` automatically. To go back to main when you're done:
...
Some projects vendor gstack by copying it into the repo (no `.git` inside the copy). Project-local skills take priority over global, so you need to update the vendored copy too. This is a three-step process:
...
To revert to main later, repeat steps 1-2 with `git checkout main && git pull` instead of `git checkout origin/<branch>`.
